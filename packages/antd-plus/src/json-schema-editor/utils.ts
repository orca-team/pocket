...
export function toTypeScriptDefinition(value: JsonValueType, options: ToTypeScriptDefinitionOptions = {}) {
    ...
    switch (root.type) {
      case 'object':
        ...
        (root.items || []).forEach((objValue) => {
          ...
          res.push(`${tab(level + 1)}${objValue.name}${objValue.required ? '' : '?'}: ${rev(objValue, level + 1)};`);
        });
        res.push(`${tab(level)}}`);
        break;
      case 'array':
